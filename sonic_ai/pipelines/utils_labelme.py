...
def _annotation(annotations, bboxes_list, keypoints_list, json_path, ismakeKeypoint, join_num, category_id, idx):
    ...
    if ismakeKeypoint != 1:
        ...
        if len(keypoints_list) > join_num * len(bboxes_list):
            print('you loss {} keypoint(s) with file {}'.format(
                join_num * len(bboxes_list) - len(keypoints_list),
                json_path))
            print('Please check ！！！')
            sys.exit()
    i = 0
    for object in bboxes_list:
        ...
        if ismakeKeypoint != 1:
            ...
            for keypoint in keypoints_list[i * join_num: (i + 1) * join_num]:
